...
const validation = require('./Validator/EmployeeValidation')
...
exports.create = async (req, res) => {
	...
	try {
		...
		if (isEmailExists) return res.status(400).json({ message: 'Email already used' })
		...
		const employee = await new Employee(form).save()
